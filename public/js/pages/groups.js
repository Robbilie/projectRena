    ...
    if(r.cancreate.length === 0)
		...
		for(var i = 0; i < r.owned.length; i++)
			if(r.owned[i].owner !== null || (r.owned[i].owner === null && coreStatus.isAdmin))
				el1.appendChild(createElement(tmpl.format([r.owned[i].id,(r.owned[i].custom == 1 ? '[custom] ' : '') + r.owned[i].name, ""])));
		for(var j = 0; j < r.corporation.length; j++)
			el2.appendChild(createElement(tmpl.format([r.corporation[j].id,(r.corporation[j].custom == 1 ? '[custom] ' : '') + r.corporation[j].name, '<span class="fr">[' + ((r.groups.indexOf(parseInt(r.corporation[j].id)) >= 0) ? "member" : (r.corporation[j].custom ? '<span onclick="apply(' + r.corporation[j].id + ');">apply</span>' : "static")) + ']</span>'])));
		for(var k = 0; k < r.alliance.length; k++)
			el3.appendChild(createElement(tmpl.format([r.alliance[k].id,(r.alliance[k].custom == 1 ? '[custom] ' : '') + r.alliance[k].name, '<span class="fr">[' + ((r.groups.indexOf(parseInt(r.alliance[k].id)) >= 0) ? "member" : (r.alliance[k].custom ? '<span onclick="apply(' + r.alliance[k].id + ');">apply</span>' : "static")) + ']</span>'])));
